import { CalendarDays, CalendarPlus, Clock, LogOut, Stethoscope, UserRound } from 'lucide-react';
      ...
      <header className="border-b border-blush bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-md bg-lavender text-white">
          ...
          <nav className="flex max-w-full items-center gap-1 overflow-x-auto">
            ...
            <button
              className="inline-flex h-10 shrink-0 items-center gap-2 rounded-md px-3 text-sm font-medium text-slate-600 hover:bg-mist"
              onClick={onLogout}
              title="Cerrar sesion"
              type="button"
            >
    ...
    <NavLink
      className={({ isActive }) =>
        `inline-flex h-10 shrink-0 items-center gap-2 rounded-md px-3 text-sm font-medium ${
          isActive ? 'bg-blush text-ink' : 'text-slate-600 hover:bg-mist'
        }`
      }
      end={to === '/'}
      to={to}
